...
uniq(names, callBack => {
  console.log(`The new names array with all the duplicate items removed is `+ callBack)
})
...
const identifier = arr => {
  const filtered = arr.filter(obj => obj.programmer) //if obj.programmer exists, filter out.  Otherwise, skip
  return filtered[0] //returns first value of the array
}
